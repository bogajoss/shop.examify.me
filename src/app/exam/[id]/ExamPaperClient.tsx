...
import { Question, FreeExam } from '@/data/mockData';
...
export default function ExamPaperClient({ examInfo, questions, id }: ExamPaperClientProps) {
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col">
      <header className="sticky top-0 z-50 bg-background/95 backdrop-blur-md border-b border-border shadow-sm">
        <div className="mx-auto max-w-4xl px-4 py-4 flex justify-between items-center">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="font-bold text-primary line-clamp-1">{examInfo.title}</h1>
              <p className="text-[10px] text-muted-foreground uppercase tracking-wider">{examInfo.subject} • {questions.length} Questions</p>
            </div>
          </div>
          
          <div className="flex items-center gap-6">
            <div className="hidden sm:flex items-center gap-2 bg-muted px-3 py-1.5 rounded-full border border-border">
              <span className="text-[10px] font-bold text-muted-foreground uppercase">Practice Mode</span>
              <button 
                onClick={() => setIsPracticeMode(!isPracticeMode)}
                className={`w-8 h-4 rounded-full transition-colors relative ${isPracticeMode ? 'bg-primary' : 'bg-muted-foreground/30'}`}
              >
                <div className={`absolute top-0.5 w-3 h-3 rounded-full bg-white transition-transform ${isPracticeMode ? 'left-4.5' : 'left-0.5'}`} />
              </button>
            </div>

            <div className="text-right">
              <div className={`text-xl font-mono font-bold ${timeLeft < 60 ? 'text-destructive animate-pulse' : 'text-foreground'}`}>
                {formatTime(timeLeft)}
              </div>
      ...
      <main className="flex-1 mx-auto max-w-4xl px-4 py-8 w-full space-y-6">
        {questions.map((q: Question, idx: number) => (
          <div key={q.id} id={`q-${q.id}`} className="bg-card border border-border rounded-xl p-6 shadow-sm scroll-mt-32">
            <h3 className="font-semibold text-lg mb-4 flex gap-3 text-foreground">
              <span className="bg-primary/10 text-primary w-8 h-8 rounded-full flex items-center justify-center shrink-0 text-sm font-bold">{idx + 1}</span>
              {q.q}
            </h3>
