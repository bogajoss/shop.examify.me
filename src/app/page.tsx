...
import { db, Course, FreeExam, QuestionBankItem } from '@/data/mockData';
...
export default function Home() {
  ...
  const features = [
    { icon: <Monitor className="h-5 w-5" />, title: 'অনলাইন প্রোগ্রাম', desc: 'ঘরে বসেই সেরা প্রস্তুতি।' },
    { icon: <Users className="h-5 w-5" />, title: 'অভিজ্ঞ শিক্ষকবৃন্দ', desc: 'সেরা মেন্টরদের সান্নিধ্যে।' },
    { icon: <BookOpen className="h-5 w-5" />, title: 'স্টাডি ম্যাটেরিয়ালস', desc: 'মানসম্মত নোট এবং রিসোর্স।' },
    { icon: <Lightbulb className="h-5 w-5" />, title: 'কনসেপ্ট ভিত্তিক ক্লাস', desc: 'বেসিক হোক শক্তিশালী।' },
    { icon: <FileText className="h-5 w-5" />, title: 'ইউনিক এক্সাম সিস্টেম', desc: 'নিজেকে যাচাইয়ের সেরা মাধ্যম।' },
    { icon: <MessageCircle className="h-5 w-5" />, title: 'Q&A সাপোর্ট', desc: 'তাৎক্ষণিক সমস্যার সমাধান।' },
    { icon: <Smartphone className="h-5 w-5" />, title: 'সঠিক গাইডলাইন', desc: 'সাফল্যের পথে এগিয়ে চলুন।' },
    { icon: <BarChart className="h-5 w-5" />, title: 'এক্সাম লিডারবোর্ড', desc: 'অন্যদের সাথে নিজের অবস্থান যাচাই।' }
  ];

  const nextSlide = () => setSlideIndex((prev) => (prev + 1) % 5);
  const prevSlide = () => setSlideIndex((prev) => (prev - 1 + 5) % 5);

  const filteredQbItems = db.questionBanks.filter(item => item.category === activeQbTab);
        ...
        <section className="w-full bg-secondary/30 py-10 md:py-24 lg:py-32 px-4 border-b border-border relative overflow-hidden">
          ...
          <div className="mx-auto max-w-5xl text-center space-y-8">
            ...
            <h1 className="text-3xl sm:text-5xl lg:text-7xl font-bold tracking-tight text-foreground leading-tight">
              স্বপ্ন যখন <span className="text-transparent bg-clip-text bg-gradient-to-r from-primary to-secondary-foreground">ডাক্তার</span> হওয়ার
            </h1>
            <p className="text-base sm:text-lg md:text-xl text-muted-foreground max-w-2xl mx-auto leading-relaxed px-4">
              Examify-এর সাথে আপনার মেডিকেল ভর্তি প্রস্তুতির যাত্রা শুরু হোক। শপ থেকে টোকেন সংগ্রহ করুন এবং নিজের সুবিধামতো এনরোল করুন।
            </p>
            <div className="flex flex-col sm:flex-row items-center justify-center gap-4 pt-4 px-4 w-full sm:w-auto">
              <Link href="/#courses" className="w-full sm:w-auto">
                <Button size="lg" className="w-full shadow-lg shadow-primary/20 hover:scale-105">কোর্সসমূহ দেখুন</Button>
              </Link>
              <Link href="/#free-exams" className="w-full sm:w-auto">
                <Button variant="outline" size="lg" className="w-full">ফ্রি এক্সাম দিন</Button>
              </Link>
            </div>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-8 mt-12 pt-8 border-t border-border/50 max-w-4xl mx-auto px-4">
              {[
                ...
                <div key={stat.label} className="flex flex-col items-center p-3 sm:p-4 rounded-lg bg-card/50 hover:border-primary/10 transition-all border border-transparent">
        ...
        <div className="mx-auto flex max-w-7xl flex-col gap-12 sm:gap-16 px-4 pb-16 pt-10 sm:pt-14 w-full">
          ...
          <section id="courses" className="space-y-6 w-full scroll-mt-24">
            <div className="flex flex-col gap-6">
              <div className="flex items-end justify-between gap-4">
                <div>
                  <h2 className="text-2xl sm:text-3xl font-semibold tracking-tight text-foreground">চলমান কোর্সসমূহ</h2>
                  <p className="text-sm text-muted-foreground">আপনার সফলতার জন্য বিশেষভাবে ডিজাইন করা প্রিমিয়াম প্রোগ্রাম।</p>
                </div>
              </div>
              
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                {db.courses.map((course) => (
                  <div key={course.id} className="rounded-lg bg-card text-card-foreground overflow-hidden border border-border shadow-sm hover:shadow-md hover:shadow-primary/5 transition-all flex flex-col h-full group">
                    <div className="w-full aspect-video relative overflow-hidden bg-muted">
                      ...
                      <div className="absolute top-2 right-2 flex flex-col gap-1 items-end">
                        <div className="inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-semibold bg-primary/90 text-white shadow-sm">{course.batch}</div>
                      </div>
                    </div>
                    <div className="flex-1 p-5 flex flex-col justify-between gap-4">
                      <div>
                        <h3 className="text-lg font-bold mb-2 leading-tight group-hover:text-primary transition-colors">{course.title}</h3>
                        <p className="text-muted-foreground text-xs mb-4 line-clamp-3">{course.description}</p>
                        <div className="grid grid-cols-2 gap-2 text-xs text-muted-foreground">
                          {['লাইভ ক্লাস', 'লেকচার নোট', 'স্ট্যান্ডার্ড এক্সাম', 'সলভ শিট'].map((f) => (
                            <div key={f} className="flex items-center gap-1 text-primary">
                              <Check className="h-3 w-3" /> <span className="text-muted-foreground">{f}</span>
                            </div>
                          ))}
                        </div>
                      </div>
                      <div className="flex items-center justify-between gap-2 mt-auto pt-4 border-t border-dashed border-border">
                        <div className="flex flex-col items-start">
                          <span className="text-[10px] text-destructive line-through font-medium">৳{course.oldPrice}</span>
                          <div className="text-base font-bold text-primary">৳{course.price}</div>
                        </div>
          ...
          <section id="free-exams" className="space-y-6 w-full scroll-mt-24">
            <div className="flex flex-col gap-6">
              <div className="flex items-end justify-between gap-4">
                <div>
                  <h2 className="text-2xl sm:text-3xl font-semibold tracking-tight text-foreground">ফ্রি এক্সামসমূহ</h2>
                  <p className="text-sm text-muted-foreground">নিজেদের যাচাই করুন সম্পূর্ণ বিনামূল্যে।</p>
                </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                {db.freeExams.map((exam) => (
                  <div key={exam.id} className="rounded-lg bg-card text-card-foreground overflow-hidden border border-border shadow-sm hover:shadow-md hover:shadow-primary/5 transition-all flex flex-col h-full group">
                    <div className="w-full aspect-video relative overflow-hidden bg-muted flex items-center justify-center text-muted-foreground/30">
                      ...
                      <div className="absolute top-2 right-2">
                        <span className="inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-semibold bg-primary text-primary-foreground shadow-sm">Free</span>
                      </div>
                      <div className="absolute bottom-2 left-2">
                        <span className="inline-flex items-center rounded px-2 py-0.5 text-[10px] font-medium bg-foreground/80 text-background backdrop-blur-md">{exam.subject}</span>
                      </div>
                    </div>

                    <div className="flex-1 p-5 flex flex-col justify-between gap-4">
                      <div>
                        <h3 className="text-lg font-bold mb-2 leading-tight group-hover:text-primary transition-colors line-clamp-2">{exam.title}</h3>
                        ...
                        <div className="grid grid-cols-2 gap-y-2 gap-x-4 text-xs text-muted-foreground bg-muted/40 p-3 rounded-lg border border-border/50">
                          <div className="flex items-center gap-1.5"><Clock className="h-3 w-3 text-primary" /> {exam.time}</div>
                          <div className="flex items-center gap-1.5"><FileText className="h-3 w-3 text-primary" /> {exam.questions} Ques.</div>
                          <div className="flex items-center gap-1.5"><Check className="h-3 w-3 text-primary" /> {exam.questions} Marks</div>
                          <div className="flex items-center gap-1.5"><span className="text-destructive font-bold">-</span> 0.25 Neg.</div>
                        </div>
                      </div>
          ...
          <section id="question-bank" className="space-y-6 w-full pt-4 scroll-mt-24">
            <div className="flex flex-col gap-6">
              <div className="flex items-end justify-between gap-4">
                <div>
                  <h2 className="text-2xl sm:text-3xl font-semibold tracking-tight text-foreground">প্রশ্নব্যাংক</h2>
                  <p className="text-sm text-muted-foreground">বিগত বছরের প্রশ্ন ও মডেল টেস্ট অনুশীলন করুন।</p>
                </div>
              </div>
              ...
              <div className="min-h-[40vh]">
                {filteredQbItems.length > 0 ? (
                          ...
                          {item.isLive && (
                            <div className="absolute top-2 right-2">
                              <span className="flex items-center gap-1 bg-destructive text-destructive-foreground text-[10px] font-bold px-2 py-1 rounded-full animate-pulse shadow-sm backdrop-blur-md bg-opacity-90">
                                <span className="h-2 w-2 rounded-full bg-white animate-ping"></span> Live
                              </span>
                            </div>
                          )}
                  ...
                  <div className="text-center py-12 border-2 border-dashed border-border rounded-xl">
                    ...
                    <p className="text-muted-foreground text-sm">এই ক্যাটাগরিতে কোনো প্রশ্ন পাওয়া যায়নি।</p>
                  </div>
                )}
              </div>
            </div>
          ...
          <section className="space-y-6">
            <div className="text-center md:text-left">
              ...
              <p className="text-sm text-muted-foreground">কেন বাছবেন Examify?</p>
            </div>
            <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-4 gap-4">
              {features.map((f, i) => (
                <div key={i} className="rounded-lg bg-card text-card-foreground shadow-sm border-2 border-primary/10 hover:border-primary/30 transition-colors group">
                  <div className="flex flex-col items-center text-center p-4 gap-2">
                    ...
                    <div>
                      <h3 className="font-semibold text-sm group-hover:text-primary transition-colors">{f.title}</h3>
          ...
          <section id="reviews" className="space-y-8 w-full overflow-hidden">
            <h2 className="text-2xl font-semibold tracking-tight text-center">শিক্ষার্থীদের মতামত</h2>
            <div className="relative md:px-12">
              <div className="relative w-full overflow-hidden">
                ...
                >
                  {[1, 2, 3, 4, 5].map((_, i) => (
                    <div key={i} className="min-w-full md:min-w-[50%] lg:min-w-[33.333%] px-4">
                        ...
                        <div className="p-6 flex flex-col gap-4 h-full bg-gradient-to-br from-card to-secondary/30">
                          <div className="flex items-center gap-4">
                            ...
                            <div className="flex-1 min-w-0">
                              <h3 className="font-bold text-base truncate text-primary">Student {i+1}</h3>
                              <p className="text-xs text-muted-foreground uppercase tracking-wide truncate">College Name</p>
                          ...
                          <p className="text-sm text-muted-foreground italic leading-relaxed line-clamp-4">
                            "Examify-এর মাধ্যমে টোকেন কিনে এনরোল করা খুব সহজ। তাদের সার্ভিস অসাধারণ!"
                          </p>
                        </div>
                      </div>
                    </div>
              ...
              <button onClick={prevSlide} className="absolute left-0 top-1/2 -translate-y-1/2 h-8 w-8 rounded-full border-[3px] border-input bg-background hidden md:flex items-center justify-center text-primary shadow-sm hover:bg-accent -ml-4 lg:-ml-12">
                <ArrowLeft className="h-4 w-4" />
              </button>
              <button onClick={nextSlide} className="absolute right-0 top-1/2 -translate-y-1/2 h-8 w-8 rounded-full border-[3px] border-input bg-background hidden md:flex items-center justify-center text-primary shadow-sm hover:bg-accent -mr-4 lg:-mr-12">
